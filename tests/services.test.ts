...
import { formatGithubForumSyncResult, isConfiguredGuildThread } from "../src/commands.js";
...
describe("GitHub issue body", () => {
  ...
  it("stores Discord images in R2 before creating the GitHub issue", async () => {
...
describe("Discord command boundaries", () => {
  it("accepts command targets only from the configured guild", () => {
    expect(isConfiguredGuildThread({ guildId: "akron" }, "akron")).toBe(true);
    expect(isConfiguredGuildThread({ guildId: "foreign" }, "akron")).toBe(false);
  });
});
