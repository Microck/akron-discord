...
function buildForumExampleMessage(spec: ReturnType<typeof buildForumExampleSpecs>[number]) {
  ...
  if (spec.includeCapture) {
    files.push(new AttachmentBuilder(buildPlaceholderCaptureBytes(), { name: "forsaken-city-room-capture.png" }));
  }
...
function buildExampleAkrBytes(spec: ReturnType<typeof buildForumExampleSpecs>[number]): Buffer {
  const manifest = {
    ...
    target: {
      ...
      displayName: "Forsaken City (Chapter 1)"
    },
...
function exampleProfileData(section: string): unknown {
  if (section === "StartPos") {
    return {
      startPositions: [
        { room: "1-forsaken-city/00", label: "City entrance", x: 48, y: 128 },
        { room: "1-forsaken-city/03", label: "Traffic block practice", x: 176, y: 96 }
      ]
    };
  }
  if (section === "AutoKill") {
    return {
      zones: [
        { room: "1-forsaken-city/04", label: "Retry pit", x: 112, y: 144, width: 64, height: 24 }
      ]
    };
  }
  if (section === "AutoDeafen") {
    return {
      zones: [
        { room: "1-forsaken-city/06", label: "Focus room", x: 32, y: 48, width: 160, height: 120 }
      ]
    };
  }
  if (section === "Keybinds") {
